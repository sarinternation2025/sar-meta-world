...
class DeployCommand {
  constructor() {
    this.deployCommand = new Command("deploy").description(
      "Deploy and manage application deployments (admin only)",
    );
  ...
  setupSubcommands() {
    ...
    this.deployCommand
      ...
      .action(
        AdminUtils.createAdminCommand(
          "deploy start",
          this.deployStart.bind(this),
          {
            requireConfirmation: true,
            action: "start application deployment",
            warning:
              "This will deploy the application to the specified environment and may cause downtime.",
          },
        ),
      );
    ...
    this.deployCommand
      ...
      .action(
        AdminUtils.createAdminCommand(
          "deploy rollback",
          this.deployRollback.bind(this),
          {
            requireConfirmation: true,
            action: "rollback deployment",
            warning:
              "This will rollback the application to a previous version and may cause downtime.",
          },
        ),
      );
   ...
   */
  async deployStart(options, _command) {
    ...
    try {
      ...
      if (config.get("deploy.rollbackOnFailure", true)) {
        ...
        await this.deployRollback({ environment }, _command);
      }
   ...
   */
  async deployStatus(options, _command) {
   ...
   * Rollback deployment
   */
  async deployRollback(options, _command) {
   ...
   */
  async deployHistory(options, _command) {
    ...
    history.slice(0, parseInt(limit)).forEach((deployment, index) => {
      const statusColor =
        deployment.status === "success" ? chalk.green : chalk.red;
      console.log(
        `${index + 1}. Version: ${chalk.cyan(deployment.version)} | Status: ${statusColor(deployment.status)} | Duration: ${chalk.yellow(deployment.duration)} | Date: ${chalk.gray(deployment.date)}`,
      );
    });
  }
   ...
   */
  async deployConfig(options, _command) {
