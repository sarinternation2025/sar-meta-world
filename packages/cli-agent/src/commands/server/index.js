...
const chalkColors = {
  blue: typeof chalk.blue === 'function' ? chalk.blue : (text) => `\x1b[34m${text}\x1b[0m`,
  green: typeof chalk.green === 'function' ? chalk.green : (text) => `\x1b[32m${text}\x1b[0m`,
  red: typeof chalk.red === 'function' ? chalk.red : (text) => `\x1b[31m${text}\x1b[0m`,
  yellow: typeof chalk.yellow === 'function' ? chalk.yellow : (text) => `\x1b[33m${text}\x1b[0m`,
  cyan: typeof chalk.cyan === 'function' ? chalk.cyan : (text) => `\x1b[36m${text}\x1b[0m`,
  magenta: typeof chalk.magenta === 'function' ? chalk.magenta : (text) => `\x1b[35m${text}\x1b[0m`,
  gray: typeof chalk.gray === 'function' ? chalk.gray : (text) => `\x1b[90m${text}\x1b[0m`
};
...
class ServerCommand {
  constructor() {
    this.serverCommand = new Command('server')
  ...
  async startServer(options, command) {
    ...
    const mockServerStart = () => {
      ...
      console.log(chalkColors.gray(`Server is running on http://localhost:${serverPort}`));
  ...
  async serverLogs(options, command) {
    ...
    mockLogs.slice(-parseInt(lines)).forEach(log => {
      const level = log.includes('[INFO]') ? chalkColors.blue : 
                    log.includes('[WARN]') ? chalkColors.yellow :
                    log.includes('[ERROR]') ? chalkColors.red : chalkColors.gray;
    ...
    if (follow) {
      ...
      const followInterval = setInterval(() => {
        ...
        console.log(chalkColors.blue(`[${timestamp}] [INFO] Heartbeat - Server is running`));
      }, 5000);
