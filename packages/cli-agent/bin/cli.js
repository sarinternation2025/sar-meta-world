...
program
  .name("sar-cli")
  .description(
    "SAR Meta World CLI Agent - A comprehensive command-line interface for managing SAR projects",
  )
...
program.on("--help", () => {
  console.log("");
  console.log(
    chalk.cyan(figlet.textSync("SAR CLI", { horizontalLayout: "full" })),
  );
  console.log("");
...
function addCommandsToProgram() {
  try {
    ...
    if (
      ...
      backupCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      configCommand.constructor.name === "Command"
    ) {
    ...
    if (dbCommand && typeof dbCommand === "object") {
      if (
        dbCommand.dbCommand &&
        dbCommand.dbCommand.constructor &&
        dbCommand.dbCommand.constructor.name === "Command"
      ) {
        // Add admin check to db command
        const dbCmd = dbCommand.dbCommand;
        dbCmd.hook("preAction", (_thisCommand, _actionCommand) => {
    ...
    if (
      ...
      deployCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      const dockerCmd = new Command("docker")
        ...
        .action(() => {
          dockerCommand.checkAdminAccess();
          console.log(
            chalk.blue("Docker command executed - implementation needed"),
          );
        });
    ...
    if (
      ...
      gitCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      const monitorCmd = new Command("monitor")
        ...
        .action(() => {
          monitorCommand.checkAdminAccess();
          console.log(
            chalk.yellow("Monitor command executed - implementation needed"),
          );
        });
    ...
    if (
      ...
      optimizeCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      pluginCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      projectCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      securityCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      serverCommand.constructor.name === "Command"
    ) {
    ...
    if (
      templateCommand &&
      templateCommand.constructor &&
      templateCommand.constructor.name === "Command"
    ) {
    ...
    if (
      ...
      testCommand.constructor.name === "Command"
    ) {
    ...
    const aiCommand = new Command("ai")
      ...
      .action(() => {
        ...
        console.log(
          chalk.gray("This feature will provide AI-powered assistance for:"),
        );
...
async function main() {
  try {
    ...
    if (process.argv.length === 2) {
      ...
      console.log("");
      console.log(
        chalk.green(`SAR Meta World CLI Agent v${packageJson.version}`),
      );
      console.log(
        chalk.gray(
          "A comprehensive command-line interface for managing SAR projects",
        ),
      );
      console.log("");
